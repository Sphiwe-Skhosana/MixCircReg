...
fit_circ_regression=function(theta,x1,x2,initBeta1=NULL,initBeta2=NULL,initmu=NULL,initlmd=NULL,initprop=NULL){
  ...
  while(count<R){
    ...
    if(!is.null(fit) & !any(fit$lmd==0) & !any(fit$Beta1< -5) & !any(fit$Beta1> 5)&!any(fit$Beta2< -5) & !any(fit$Beta2> 5)) {count=count+1;res[[count]]=fit;}
    counter=counter+1
    if(counter==1e2) count=R
  }
  return(res)
}
###Simulating from a circular regression model using the von-Mises distribution
library(circular)
n=100
mu=pi/2
kp=10
